...
func (r *repository) AddActivityInfo(activity string, duration time.Duration, calories int) error {
	_, err := r.db.Exec(`INSERT INTO activity(activity, duration, calories) VALUES ($1, $2, $3)`, activity, time.Unix(0, duration.Nanoseconds()), calories)
	return errors.Wrap(err, "exec query")
}

func (r *repository) GetSumDuration() (time.Duration, error) {
	row := r.db.QueryRow(`SELECT SUM(duration) FROM activity`)
	...
	return time.Duration(activityTime.Nanosecond()), nil
}

func (r *repository) GetSumCalories() (int, error) {
	row := r.db.QueryRow(`SELECT SUM(calories) FROM activity`)
	if err := row.Err(); err != nil {
